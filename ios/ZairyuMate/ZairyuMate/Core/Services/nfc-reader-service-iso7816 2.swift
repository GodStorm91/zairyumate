    ...
    nonisolated func tagReaderSession(_ session: NFCTagReaderSession, didInvalidateWithError error: Error) {
        Task { @MainActor in
            ...
            if let nfcError = error as? NFCReaderError {
                if case NFCReaderError.Code.readerSessionInvalidationErrorUserCanceled = nfcError.code {
                    continuation.resume(throwing: NFCReaderError.userCancelled)
                    return
                }
                if case NFCReaderError.Code.readerSessionInvalidationErrorSessionTimeout = nfcError.code {
